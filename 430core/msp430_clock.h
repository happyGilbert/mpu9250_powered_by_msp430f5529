 ...
 *                  required for eMPL.
 *      @details    MCLK, and SMCLK are all sourced directly by the DCO.
 *                  The DCO frequency is set by multiplying the internal
 *                  32.768kHz oscillator(REFOCLK), which may vary in performance
 *                  between multiple chips. This may be an issue for time-critical
 *                  tasks such as providing a baud rate reference.
 *                  ACLK is  sourced from REFOCLK: internal 32.768kHz oscillator.
 */
 ...
 *  @param[in]  xt      1 if XT1 is present, 2 if XT2 is present, 3 if XT1 and XT2 is present, 0 otherwise.
